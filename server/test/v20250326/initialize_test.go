...
func TestInitializeResponse_v20250326(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:            "2025-03-26 version request",
			clientVersion:   "2025-03-26",
			expectedVersion: "2025-03-26",
		},
		{
			name:            "version with v prefix",
			clientVersion:   "v2025-03-26",
			expectedVersion: "2025-03-26",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// 2025-03-26 specific validations
			// Should NOT have sampling capability (that's for clients)
			if _, hasSampling := capabilities["sampling"]; hasSampling {
				t.Errorf("Server should not declare sampling capability in 2025-03-26 spec")
			}

			// Optional instructions field is allowed in 2025-03-26
			if instructions, hasInstructions := result["instructions"]; hasInstructions {
				if _, ok := instructions.(string); !ok {
					t.Errorf("Expected instructions to be a string if present, got %T", instructions)
				}
			}
		})
	}
...
func TestInitializeResponseWithoutRegistrations_v20250326(t *testing.T) {
